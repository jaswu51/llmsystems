...
def _split_module(modules: nn.Sequential) -> Tuple[List[nn.Sequential], List[torch.device]]:
    ...
    for name, module in modules.named_children():
        ...
        # Check if module is wrapped with WithDevice
        if isinstance(module, WithDevice):
            device = module.device
        
        # If this is the first module or device changed, start a new partition
        if current_device is None or device != current_device:
            # Save the previous partition if it exists
            if current_partition and current_device is not None:
        ...
        # Add module to current partition (not as a tuple)
