   ...
   */
    ...
    for (int t = 0; t < (a_shape[2] + TILE - 1) / TILE; t++) {
        if (row < a_shape[1] && (t * TILE + threadIdx.y) < a_shape[2]) {
            a_shared[threadIdx.x][threadIdx.y] = a_storage[
        ...
        if ((t * TILE + threadIdx.x) < b_shape[1] && col < b_shape[2]) {
            b_shared[threadIdx.x][threadIdx.y] = b_storage[
    ...
    if (row < a_shape[1] && col < b_shape[2]) {
        out[batch * (a_shape[1] * b_shape[2]) + row * b_shape[2] + col] = sum;
    }
}
...
__global__ void mapKernel(
   ...
   */
    ...
    out[pos] = fn(fn_id, in_storage[index_to_position(in_index, in_strides, shape_size)], 0);
